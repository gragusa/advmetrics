...
1.  Use `R` to generate $n=200$ observations from the following model $$y_{i}=\beta_{0}+\beta_{1}x_{i}+u_{i},\quad n=1,\ldots,200$$ where $x_{i}$ and $u_{i}$ are independent and $(u_{i},x_{i})$ is [independent]{.ul} of $(u_{j},x_{j})$ for $i\neq j$; $$u_{i}\sim\chi_{3}^{2},\quad x_{i}\sim N(0,1),$$ and $\beta_{0}=0.5$ and $\beta_{1}=0.3$. ($\chi_{p}^{2}$ is the chi squared distribution with $p$ degrees of freedoms).
